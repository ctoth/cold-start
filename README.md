...
- **`cold_start/integers.py`** — the Grothendieck bridge, the quotient
  ...
  A 28-node bridge, **every obligation paid** (toll: 148,761) — including
...
- **`cold_start/ring_z.py`** — **the full commutative ring of integers
  ...
  A 51-node bridge, **all 23 obligations paid** (toll: 1,331,516) — ring
  axioms, multiplicative associativity, both distributive laws, and respect
  for `·` among them. The engine is `ring_nf.elaborate_combination`: linear
  combinations of hypotheses with *term coefficients*, normalized by the one
  sparse polynomial owner under `PEANO_SEMIRING_CONTEXT` and closed by its
  proved cancellation recipe.
...
- [x] The full commutative ring ℤ into PEANO (`cold_start.ring_z`):
      ...
      riding `ring_nf`'s natural-semiring normal form and term-coefficient
