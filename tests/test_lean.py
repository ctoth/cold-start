...
def test_corpus_carries_every_proof_and_needs_no_imports():
    text = export_corpus()
...
def test_corpus_epilogue_instantiates_the_arithmetic_theorems_at_Nat():
    text = export_corpus()
...
def test_the_committed_corpus_file_is_up_to_date():
...
def test_write_corpus_writes_what_export_corpus_returns(tmp_path):
    target = tmp_path / "ColdStart.lean"
    assert write_corpus(target) == target
    assert target.read_text(encoding="utf-8") == export_corpus()
...
def test_generated_corpus_compiles_under_lean(tmp_path):
...
def test_lean_rejects_a_corrupted_export(tmp_path):
    ...
    found = _lean4_executable()
    if found is None:
        pytest.skip("no Lean 4 toolchain on PATH")
    exe, _version = found
    broken = export_corpus().replace("(ax_add_zero zero)", "(Eq.refl zero)", 1)
    assert broken != export_corpus()
